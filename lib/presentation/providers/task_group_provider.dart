...
class TaskGroupProvider extends ChangeNotifier {
  ...
  void addTaskGroup(TaskGroup taskGroup) async {
    ...
    await taskGroupRepo!
        .updateTaskGroups(_groups, add: true, id: taskGroup.taskGroupId);
    notifyListeners();
  }
  ...
  void updateBonusTime({Duration? duration, bool reset = false}) async {
    if (reset)
      ...
      _current!.bonusTime = _current!.bonusTime! + duration!;
    }
  ...
  int get tasksCompleted => _groups!
      .fold<int>(0, (previous, element) => previous + element.completedCount);
